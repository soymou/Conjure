...
function buildPrompt(text: string) {
  return `Eres una asistente jurídica que explica jurisprudencia mexicana a personas no especialistas. Resume el extracto en lenguaje llano (máximo 3 párrafos), resalta los puntos clave en frases tipo viñeta y evita copiar literalmente, salvo que sea absolutamente necesario.\n\nTexto original:\n${text}`;
}
...
export async function POST(req: Request) {
  ...
  if (!huggingFaceKey) {
    return NextResponse.json(
      { error: "HUGGINGFACE_API_KEY no está configurada en el entorno del servidor." },
      { status: 503 }
    );
  }
  ...
  if (!text) {
    return NextResponse.json({ error: "Se requiere texto para generar la explicación." }, { status: 400 });
  }
  ...
  if (!summary) {
    return NextResponse.json(
      { error: "Hugging Face no devolvió una explicación válida." },
      { status: 502 }
    );
  }
